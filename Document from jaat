...
app.get('/', (req, res) => {
    ...
    <html>
    <head>
        <title>Waleed Bot Configuration</title>
        <style>
    ...
    <body>
        <h1>Waleed Bot Configuration</h1>
        <form id="configForm">
            ...
            <div class="form-group">
                ...
                <input type="text" id="prefix" name="prefix" value="/waleed" required>
            </div>
            <div class="form-group">
...
app.post('/configure', (req, res) => {
    try {
        ...
        config.prefix = req.body.prefix || '/waleed';
...
app.listen(PORT, () => {
    console.log(`Waleed Bot running on port ${PORT}`);
    try {
...
async function initializeBot() {
    ...
    console.log('Initializing Waleed Bot with WS3-FCA...');
    ...
    try {
        ...
        console.log('Waleed Bot is now listening for events...');
    } catch (err) {
...
async function handleMessage(api, event) {
    ...
    if (mentions && mentions.some(m => m.id === config.adminID)) {
        const abuses = [
            "Please respect the admin!",
            "Admin ko respect karo!",
    ...
    try {
        switch (command) {
            ...
            default:
                if (!isAdmin) {
                    await api.sendMessage("Invalid command! Use " + config.prefix + " for help.", threadID);
                } else {
                    await api.sendMessage(`Bot prefix is: ${config.prefix}`, threadID);
                }
        }
    } catch (err) {
...
async function handleGroupCommand(api, event, args, isAdmin) {
    ...
    if (!isAdmin) {
        await api.sendMessage("Admin access required!", threadID);
        return;
    }
...
async function handleThreadNameChange(api, event) {
    ...
    if (lockedGroups[threadID] && authorID !== config.adminID) {
        ...
        await api.sendMessage(`Group name is locked! Only admin can change it. @${authorID}`, threadID);
    }
}
...
async function handleNicknameChange(api, event) {
    ...
    if (lockedNicknames[threadID] && authorID !== config.adminID) {
        ...
        await api.sendMessage(`Nicknames are locked! Only admin can change them. @${authorID}`, threadID);
    }
}